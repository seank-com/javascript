...
async function main(argc, argv) {
  ...
  if (argc < 3) {
    console.log('Usage: node dumpManifest.js <provider.json>');
    return;
  }

  const provider = await load(argv[2]);

  const resourceProvider = provider.namespace;

  console.log(`# ${resourceProvider}`);

  for (var i = 0; i < provider.resourceTypes.length; i++) {
    const resourceType = provider.resourceTypes[i];

    if (resourceType.capabilities.includes("SupportsLocation") || resourceType.capabilities.includes("SupportTags")) {
      console.log(`${resourceProvider}/${resourceType.resourceType}`);
    }
  }
}
